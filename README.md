...
- [Setup Kali Linux VM in Hyper-V](#setup-kali-linux-vm-in-hyper-v)
...
This project/repository is a local setup of <span style="color: red; font-weight: bold;">Kali Linux VM in Hyper-V</span>.
...
- The configurations in this project/repository are for your reference ONLY (the reasons are as follows):
    - The setup is hosted in <span style="color: green; font-weight: bold;">Virtual Machine</span> environment, leveraging <span style="color: green; font-weight: bold;">Hyper-V</span> on a <span style="color: green; font-weight: bold;">Windows</span> host.
    - You can download Kali Linux Hyper-V VM from [Official Kali Pre-built Virtual Machines](https://www.kali.org/get-kali/#kali-virtual-machines).
    - This setup require you to configure two network interfaces for the Kali Linux Hyper-V VM:

        1. Default Switch
        2. Hyper-V_Lab (you can create "Internal" network in Hyper-V's Virtual Switch Manager)
...
Description:
      ...
      Usage:
             ./kali-linux-hyper-v-init.sh [options]
...
The `kali-linux-hyper-v-init.sh` file can be found [here](./kali-linux-hyper-v-init.sh) or below:

<details>

<summary><span style="padding-left:10px;">Click here to expand for the "kali-linux-hyper-v-init.sh" !!!</span></summary>
...
- Do a `apt update`
- Stop `postgresql-15`
    - This is because Kali Linux 2023.3a Hyper-V VM has `postgresql-15` installed and running that will cause `apt upgrade` FREEZE.
...
- Set hostname to `kali-hyper-v`
- Set timezone to `Asia/Singapore`
- Configure static ip for `eth1` (that is "Hyper-V_Lab" - "Internal" network in Hyper-V)
    - '192.168.138.6/24'
- Set root password
- Maps hostnames to IP addresses in `/etc/hosts`
    - '127.0.0.1       kali-hyper-v'
    - '192.168.138.6   kali-hyper-v'
- Do a `apt update`
- Install `rust`
- Install `starship`
